...
# input
sce <- readRDS('./data/day_1_processed/ctrl.rds')

sce <- seurat_clusters(sce, ndimred = 20, resolution_param = 1) # clusters used to resolve between mk and ery CMPs
...
sce$dpt_cluster = '0' # cluster containing cells with intermediate pseudotime values 
...
tmp2 <- tmp$`12`%>% as_tibble(rownames = 'ID') %>% mutate(Symbol = rowData(sce)[ID,'Symbol'])  %>% 
  dplyr::select(Symbol, FDR, AUC.10) %>% arrange(FDR) # erythroid - basophil signature
tmp3 <- tmp$`10`%>% as_tibble(rownames = 'ID') %>% mutate(Symbol = rowData(sce)[ID,'Symbol'])  %>%
  dplyr::select(Symbol, FDR, AUC.12) %>% arrange(FDR) # megakaryocytic signature
...
for (set in 1:length(lin_sets)){
  write.table(format(x = lin_sets[[set]] %>% dplyr::mutate(Symbol = rowData(sce)[gene,'Symbol']), digits =2),
              paste0('./data/day_1_processed/lin_sets/',

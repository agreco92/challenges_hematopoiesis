...
## Isolate tip points
# stem tip
stem_tip = as.integer(which.max(as.logical(sce$population == 'HSC') * reducedDim(sce,'DiffusionMap')[,2]))
sce$stem_tip = FALSE
sce$stem_tip[stem_tip] = TRUE

# mk tip
mk_tip = as.integer(which.min(as.logical(sce$population == 'CMP') * reducedDim(sce,'DiffusionMap')[,4]))
sce$mk_tip = FALSE
sce$mk_tip[mk_tip] = T
...
# ly tip
ly_tip = as.integer(which.min(as.logical(sce$cluster == 6) * reducedDim(sce,'DiffusionMap')[,3]))
sce$ly_tip =FALSE
sce$ly_tip[ly_tip] = T

# my tip
my_tip = as.integer(which.min(as.logical(sce$population == 'CMP') * reducedDim(sce,'DiffusionMap')[,1]))
sce$my_tip = FALSE
sce$my_tip[my_tip] = T
...
saveRDS(sce, './data/day_1_processed/sepsis.rds')

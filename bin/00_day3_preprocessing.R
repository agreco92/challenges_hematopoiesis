...
# preprocessing -----------------------------------------------------------
folders <- paste0(dir('./data/day_3',full.names = T),'/10x/')
sce <- read10xCounts(folders)
sce$Sample <- gsub('./data/day_3/|/10x/','',sce$Sample)
...
sce$diff_outlier <- as.logical(isOutlier(reducedDim(sce,'DiffusionMap')[,1],type = 'lower',nmads = 10)) | as.logical(isOutlier(reducedDim(sce,'DiffusionMap')[,2],type = 'higher',nmads =5))
...
# save output -------------------------------------------------------------
saveRDS(sce,'./data/day_3_processed/sce.rds')
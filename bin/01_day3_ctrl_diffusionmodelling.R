...
# remove more outliers (~20) resulting from analysis of control dataset alone
plotDiffusionMap(sce, col = 'population', ncomponents = 3, shape = 'population')
sce$diff_outlier <- as.logical(isOutlier(reducedDim(sce,'DiffusionMap')[,3],type = 'lower',nmads = 3))
plotDiffusionMap(sce, col = 'diff_outlier', ncomponents = 3, shape = 'population')
table(sce$diff_outlier)
sce <- sce[,!sce$diff_outlier]

# recompute diffusion map without outliers
dmap <- destiny::DiffusionMap(reducedDim(sce)[,1:20])
reducedDim(sce, 'DiffusionMap') = dmap@eigenvectors
...
# mk tip
mk_tip = as.integer(which.min(as.logical(sce$population == 'CMP') * reducedDim(sce,'DiffusionMap')[,2]))
sce$mk_tip =FALSE
sce$mk_tip[mk_tip] = T

# ery tip
ery_tip = as.integer(which.max(as.logical(sce$population == 'CMP') * reducedDim(sce,'DiffusionMap')[,4]))
sce$ery_tip =FALSE
...
# my tip
my_tip = as.integer(which.max(as.logical(sce$population == 'CMP') * reducedDim(sce,'DiffusionMap')[,2]))
sce$my_tip = FALSE
sce$my_tip[my_tip] = T
...
saveRDS(sce, './data/day_3_processed/ctrl.rds')

#This code reproduces the fit of the label propagation model for monocyte data
...
begin
	...
	data1=[0.0,20.9,26.7,25.6,25.1]
	data2=[0.0,23.7,32.1,29.2]
	data3=[0.0,2.36,7.62,11.2,10.3]
end
...
@model function fit_alpha(data1, prob1, data2, prob2, data3, prob3)
	...
	#
	mppfr1 ~ Uniform(20.0,40.0)
	mppfr2 ~ Uniform(20.0,40.0)
	mppfr3 ~ Uniform(0.0,20.0)
	#
	par1 = [a1, mppfr1]
	par2 = [a2, mppfr2]
	par3 = [a3, mppfr3]
...
begin
	...
	for i = 1:nruns
		...
		a3 = parmatrix.a3[i]
		#
		mppfr1_median = parmatrix.mppfr1[i]
		mppfr2_median = parmatrix.mppfr2[i]
		mppfr3_median = parmatrix.mppfr3[i]
		#
		fittedError = median(parmatrix.fittedError)
		#
		solt = collect(0.0:0.1:15.0)
		prob1 = ODEProblem(model1,f0s,t_span,[a1, mppfr1_median])
		prob2 = ODEProblem(model2,f0s,t_span,[a2, mppfr2_median])
		prob3 = ODEProblem(model3,f0s,t_span,[a3, mppfr3_median])
...
begin
	#plot results
	solt = collect(0.0:0.1:15.0)
	plot(solt,bands[:,2],legend=false,ylim=[0.0,40.0],seriescolor=:firebrick,linewidth=2,
		ribbon=(bands[:,2].-bands[:,1],bands[:,3].-bands[:,2]),
		ylabel=string("Label frequency in monocytes (%)"),xlabel="Time after treatment (days)",
